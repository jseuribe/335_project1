/********************************************************************************************************************************************************
 ...
 Purpose:An AVL tree specifically designed to handle the sequenceMap data type
...
{
  public:
    AvlTree( ) : root{ nullptr } , totalInsertionCalls{0} , totalContainsCalls{0} , totalRemoveCalls{0}, successfulRemoves{0}
      { }
  ...
  private:
    ...
    struct AvlNode
    {
        ...
        AvlNode( const sequenceMap & ele, AvlNode *lt, AvlNode *rt, int h = 0 )
          : element{ ele }, left{ lt }, right{ rt }, height{ h } { }
        
        AvlNode( sequenceMap && ele, AvlNode *lt, AvlNode *rt, int h = 0 )
    ...
    void find( sequenceMap x, AvlNode *t )
    {
     ...
     */
    void insert( sequenceMap & x, AvlNode * & t )
    {
        totalInsertionCalls++;
        if( t == nullptr )
     ...
     * Set the new root of the subtree.
     */
    void remove(  sequenceMap & x, AvlNode * & t )
    {
	totalRemoveCalls++;
     ...
     */
    bool contains( sequenceMap x, AvlNode *t )
    {
	totalContainsCalls++;
        if( t == nullptr ){
            //cout << "not found" << endl;
...
*****************************************************/
    ...
    float numberOfNodes(AvlNode *root, float value)
    {
	if(root == nullptr) return 0;
	else return numberOfNodes(root->left, value) + numberOfNodes(root->right, value) + 1;
    }

    float depth(AvlNode *root, int depthsoFar)
    {
	if(root == nullptr) return 0;
